...
using System.Collections.Generic; // Додано для використання List<T>

namespace InternHub.IntegrationTests
{
    // Необхідний клас, який має бути у проекті (посилання для TestData)
    // public static class RoleConstants { public const string Admin = "Admin"; public const string Company = "Company"; public const string Candidate = "Candidate"; }
    // public static class TestData { ... } // Клас з Guid-константами
    ...
    public class CustomWebApplicationFactory : WebApplicationFactory<Program>
    {
        protected override void ConfigureWebHost(IWebHostBuilder builder)
        {
            ...
            builder.ConfigureServices(services =>
            {
                ...
                // Використання TestAuthHandler (потрібне TestAuthHandler.cs з виправленим конструктором)
                ...
                // Скидаємо та створюємо базу даних для кожного тестового запуску
        ...
        {
            // --- ІСНУЮЧА ЛОГІКА: Створення основного набору користувачів (Admin, Company, Candidate) ---
            ...
            // Створення сутностей Candidate
            if (!db.Candidates.Any(c => c.UserId == candidateUserId))
            {
                db.Candidates.Add(new Candidate
                {
                    Id = candidateUserId, // Використовуємо UserId як Id для спрощення зв'язку з DTO
            ...
            // --- ДОПОВНЕННЯ: Створення додаткових кандидатів для тестування членства в проекті ---
            
            // Тестові ID, які мають бути визначені в класі TestData.cs
            ...
            // 1. Додатковий член проекту (MemberCandidate)
            ...
            // 2. Кандидат, що не є членом проекту (NonMemberCandidate)
            ...
            // --- ДОПОВНЕННЯ: Посів Проектів ---
            
            // 1. ProjectIdForDetails (для GET {id} та GetProjects/my)
            db.Projects.Add(new Project
            {
                ...
                ProjectMembers = new List<ProjectMember>
                {
                    ...
                    new ProjectMember { ProjectId = TestData.ProjectIdForDetails, CandidateId = memberCandidateId, Role = "Dev" } // Член
                }
            });

            // 2. ProjectIdForUpdate (для PUT {id})
            db.Projects.Add(new Project
            {
            ...
            // 3. ProjectIdForMemberRemoval (для DELETE {id}/members/{memberId})
            db.Projects.Add(new Project
            {
