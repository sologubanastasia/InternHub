    ...
    public class ProjectControllerTests : IClassFixture<CustomWebApplicationFactory>
    {
        ...
        private void EnsureTestProjectId()
        {
            if (_testProjectId == Guid.Empty)
            {
                ...
                {
                    // Якщо Test01 знову провалився (наприклад, через 500 Internal Server Error)
                    // кидаємо виняток, щоб показати, що неможливо відтворити передумову.
        ...
        // [Fact] залишаємо, щоб цей метод виконувався як тест
        [Fact]
        public async Task Test01_CreateProject_ShouldReturnCreatedAndSetOwner()
        {
            // Якщо ID вже встановлено, ми вийшли з конструктора/EnsureTestProjectId(). 
            // Виходимо, щоб уникнути дублювання запису, оскільки цей метод виконується 
            // двічі (як Test01 та як допоміжний метод).
            ...
            // Assert
            // Якщо тут виникає 500 Internal Server Error, це вказує на проблему в ProjectService.CreateProjectAsync
        ...
        {
            // Arrange
            EnsureTestProjectId(); // Тепер викликає Test01, якщо проект відсутній
            _client.DefaultRequestHeaders.Clear();
            _client.DefaultRequestHeaders.Add("X-Test-UserId", _projectOwnerId.ToString());
