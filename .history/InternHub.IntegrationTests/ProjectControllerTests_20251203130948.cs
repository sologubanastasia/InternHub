...
namespace InternHub.IntegrationTests.Controllers
{
    public class ProjectControllerTests : IClassFixture<CustomWebApplicationFactory>
    {
        ...
        private HttpClient GetClientWithUser(Guid userId, string role = RoleConstants.Candidate)
        {
            ...
            // Передача ID та ролі через заголовки
            client.DefaultRequestHeaders.Add("X-Test-UserId", userId.ToString());
            client.DefaultRequestHeaders.Add("X-Test-Role", role);
            return client;
        }

        // Допоміжний метод для створення HttpClient, який імітує відсутність автентифікації (для 401)
        private HttpClient GetUnauthenticatedClient()
        {
            var client = _factory.CreateClient(new WebApplicationFactoryClientOptions { AllowAutoRedirect = false });
            // Передача порожнього ID, щоб змусити GetUserId в контролері кинути виключення або просто не пройти
        ...
        {
            // Arrange: Використовуємо власника проектів
        ...
        public async Task GetProjects_ReturnsUnauthorized_WhenNoAuthHeaders()
        {
            ...
            // Assert
            // Оскільки [Authorize] спрацює раніше, ніж GetUserId, ми очікуємо 401
        ...
        public async Task GetProjectId_ReturnsOk_ForOwner()
        {
            ...
            Assert.Equal("Details Project", project.Title);
        ...
        public async Task GetProjectId_ReturnsForbidden_ForNonMember()
        {
            ...
            // Assert
            // Сервіс кидає UnauthorizedAccessException, контролер повертає 403 Forbid
        ...
        public async Task AddMember_ReturnsNoContent_WhenSuccessful()
        {
            ...
            var client = GetClientWithUser(TestData.OwnerCandidateId);
            var createDto = new CreateProjectDto { Title = "ForMemberAdd", Description = "Test", ProjectTechnologies = new List<Guid>() };
        ...
        public async Task RemoveMember_ReturnsBadRequest_WhenTryingToRemoveOwner()
        {
            ...
            // Assert
            // Owner не може бути видалений (InvalidOperationException у сервісі -> 400 у контролері)
        ...
        public async Task RemoveMember_ReturnsForbidden_WhenRequesterIsNotOwner()
        {
            ...
            // Assert
            // UnauthorizedAccessException у сервісі -> 403 Forbid у контролері
            Assert.Equal(HttpStatusCode.Forbidden, response.StatusCode); 
        }
    }
}