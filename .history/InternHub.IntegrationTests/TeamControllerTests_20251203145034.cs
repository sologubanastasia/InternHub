...
namespace InternHub.IntegrationTests.Controllers
{
    ...
    public class TeamControllerRealDbTests : IClassFixture<CustomWebApplicationFactory>
    {
        ...
        public TeamControllerRealDbTests(CustomWebApplicationFactory factory)
        {
            _factory = factory;
        }
        ...
        public async Task ProcessRequestForProject_AcceptsRequest_AndAddsMemberToDb()
        {
            ...
            dbContext.TeamRequests.Add(new ProjectRequest
            {
                Id = requestId,
        ...
        [Fact]
        public async Task RemoveMemberFromProject_Succeeds_AndRemovesMemberFromDb()
        {
            // Arrange
            var projectId = TestData.ProjectIdForMemberRemoval;
            var ownerUserId = OwnerCandidateId;
            var memberToRemoveUserId = TestData.MemberCandidateId; // Член, якого потрібно видалити
            
            using var dbContext = _factory.GetDbContext();
            var memberToRemoveCandidate = await dbContext.Candidates.SingleAsync(c => c.UserId == memberToRemoveUserId);
            var memberToRemoveCandidateId = memberToRemoveCandidate.Id;
            
            // 1. Перевіряємо, що член присутній перед виконанням
            var isInitiallyMember = await dbContext.ProjectMembers
                .AnyAsync(m => m.ProjectId == projectId && m.CandidateId == memberToRemoveCandidateId);
            Assert.True(isInitiallyMember);
            
            var client = GetClientWithUser(ownerUserId);

            // Act
            // Використовуємо DeleteAsync для видалення члена
            var response = await client.DeleteAsync($"/api/team/projects/{projectId}/members/{memberToRemoveUserId}");
            ...
            // Assert (DB state check)
            var isMemberAfterRemoval = await dbContext.ProjectMembers
                .AnyAsync(m => m.ProjectId == projectId && m.CandidateId == memberToRemoveCandidateId);
            
            Assert.False(isMemberAfterRemoval);
        }
    }
}