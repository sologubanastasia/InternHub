...
namespace InternHub.IntegrationTests
{
    ...
    public class ProjectControllerTests : IClassFixture<CustomWebApplicationFactory>
    {
        ...
        private static Guid _testProjectId = Guid.Empty; // Ініціалізуємо
        ...
        // 💡 Допоміжний метод для гарантованого отримання ID проекту
        private void EnsureTestProjectId()
        {
            if (_testProjectId == Guid.Empty)
            {
                ...
                // Спроба знайти проект, створений Test01, у DB
                var project = context.Projects
                    .OrderByDescending(p => p.CreatedAt) // Беремо останній створений (наш тестовий)
                ...
                {
                    // Це означає, що Test01 не спрацював. У реальному тестуванні це було б помилкою.
                    // Але для уникнення 500/404 у наступних тестах можна генерувати.
                    // В даному випадку краще дозволити Test01 провалитися і зупинити.
                    // Однак, ми залишаємо його для відновлення стану.
                    throw new InvalidOperationException("Test Project ID is missing. Test01 must be executed first.");
                }
            }
        }
        ...
        private void SeedTechnologies()
        {
            ...
            // Ця логіка створює C# та Vue.js, якщо їх немає.
            // Можливо, саме тут виникали проблеми 500, якщо не було транзакції.
            // Припускаємо, що тут помилок немає, якщо не використовується .ThenInclude
        ...
        public async Task Test01_CreateProject_ShouldReturnCreatedAndSetOwner()
        {
            ...
            // Якщо Test01 успішно завершився, _testProjectId встановлено
        ...
        public async Task Test02_GetMyProjects_ShouldReturnOnlyOwnersProjects()
        {
            ...
            // Виправлено: перевірка має бути на Contains, якщо Test01 успішний.
        ...
        public async Task Test03_GetProjectDetails_AsOwner_ShouldReturnOk()
        {
            ...
            // Assert
            response.EnsureSuccessStatusCode(); // Якщо тут 500, проблема в Include або CandidateId
            var content = await response.Content.ReadAsStringAsync();
            var details = JsonConvert.DeserializeObject<ProjectDetailsDto>(content);

            Assert.Equal("Test Project Alpha", details.Name);
            // Перевіряємо, чи був доданий власник як ProjectMember
        ...
        public async Task Test05_UpdateProject_AsOwner_ShouldReturnNoContent()
        {
            ...
            // Assert
            response.EnsureSuccessStatusCode(); // Якщо тут 500, проблема в Include/Save
        ...
        public async Task Test06_UpdateProject_AsUnauthorizedCandidate_ShouldReturnForbidden()
        {
            ...
            // Assert
            // Якщо отримано 404 замість 403, це означає, що проект не знайдено ДО перевірки авторизації.
            // Перевірка EnsureTestProjectId() має це виправити.
            Assert.Equal(HttpStatusCode.Forbidden, response.StatusCode);
        }
        ...
        public async Task Test10_RemoveMember_AsUnauthorizedCandidate_ShouldReturnForbidden()
        {
            ...
            // Спочатку додамо члена назад, якщо його було видалено в Test08 (для забезпечення передумови)
            using var scope = _factory.Services.CreateScope();
            var context = scope.ServiceProvider.GetRequiredService<InternHubDbContext>();
            if (!context.ProjectMembers.Any(m => m.ProjectId == _testProjectId && m.CandidateId == _projectMemberId))
            {
                 // Додаємо члена через API
            ...
            // Act: Видалення від імені не-власника
            _client.DefaultRequestHeaders.Clear();
            _client.DefaultRequestHeaders.Add("X-Test-UserId", _candidateUserId.ToString());
            var response = await _client.DeleteAsync($"/api/projects/{_testProjectId}/members/{_projectMemberId}");
