...
namespace InternHub.IntegrationTests.Controllers
{
    ...
    public class TechnologyControllerTests : IClassFixture<CustomWebApplicationFactory>
    {
        ...
        // --- DELETE /api/technology/{id} (Видалення) ---

        [Fact]
        public async Task RemoveTechnology_Succeeds_AndRemovesFromDb()
        {
            // Arrange: Створення тимчасової технології для видалення
            ...
            // Assert (HTTP)
            response.EnsureSuccessStatusCode(); 
            Assert.Equal(HttpStatusCode.NoContent, response.StatusCode); // Очікуємо 204
        ...
        // --- GET /api/technology (Пошук/Пагінація) ---

        [Fact]
        public async Task GetTechnologyAsync_ValidQuery_ReturnsPagedResults()
        {
            // Arrange
            // Тест дозволяє анонімно, але використовуємо клієнта з користувачем для однорідності або AnonymousClient
            var client = GetAnonymousClient(); 
            ...
            // У TestData є лише одна технологія з "C#"
