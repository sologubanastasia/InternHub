...
public async Task ProcessRequestForProject_AcceptsRequest_AndAddsMemberToDb()
{
    ...
    // 1. Отримати CandidateId
    using var setupDbContext = _factory.GetDbContext(); // Контекст для підготовки
    ...
    // setupDbContext ЗАКРИВАЄТЬСЯ завдяки using, що добре.
    ...
    // Assert (DB state check)
    // ✅ КЛЮЧОВЕ ВИПРАВЛЕННЯ: Створюємо НОВИЙ КОНТЕКСТ для перевірки стану DB.
    using var verificationDbContext = _factory.GetDbContext(); 
    var processedRequest = await verificationDbContext.TeamRequests.FindAsync(requestId); // Рядок 149
