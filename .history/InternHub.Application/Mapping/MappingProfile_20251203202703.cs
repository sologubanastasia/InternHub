...
namespace InternHub.Application.Mapping
{
    public class MappingProfile : Profile
    {
        public MappingProfile()
        {
            ...
            // 🔥 ВИПРАВЛЕНО: Змінено ProjectListDto на ProjectListItemDto
            ...
            // 🔥 ВИПРАВЛЕНО: Змінено ProjectDetailDto на ProjectDetailsDto
            CreateMap<Project, ProjectDetailsDto>()
                .ForMember(dest => dest.Technologies, opt => opt.MapFrom(src => src.ProjectTechnologies.Select(pt => pt.Technology)))
                .ForMember(dest => dest.Members, opt => opt.MapFrom(src => src.ProjectMembers)) // Це мапить до List<ProjectMemberDto> Members
                .ForMember(dest => dest.OwnerName, opt => opt.MapFrom(src => src.Candidate.User.UserName));
