...
namespace InternHub.Application.Services.Project
{
    public class ProjectService : IProjectService
    {
        ...
        public async Task RemoveMemberAsync(Guid projectId, Guid ownerId, Guid memberId)
        {
            ...
            var memberId = await _projectMemberRepository.GetByIdAsync(memberId);
            
            if(memberId == null || memberId.ProjectId != projectId) 
               throw new KeyNotFoundException("Project member not found");

            if(memeberId.CandidateId == ownerId)
               throw new InvalidOperationException("Project owner cannot be removed from the project");

            await _projectMemberRepository.Remove(projectId, memberId);
            await _projectRepository.SaveChangeAsync();   
        }
