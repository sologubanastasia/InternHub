...
using System.Linq;
using System.Threading.Tasks; // Додано using для Task
...
namespace InternHub.Application.Services.Project
{
    public class ProjectService : IProjectService
    {
        ...
        // 💡 ВИПРАВЛЕНО: Додано фільтрацію за членом проекту, використовуючи GetProjectsQuery, 
        // який тепер включає ProjectMembers.
        ...
        public async Task<ProjectDetailsDto> GetProjetDetailsAsync(Guid projectId, Guid requesterId)
        {
            ...
            // Використовуємо завантажені ProjectMembers
        ...
        public async Task AddMemberAsync(Guid projectId, Guid ownerId, AddMemberDto dto)
        {
            ...
            // Замість перевірки, чи власник, перевіряємо, чи вже є членом
        ...
        // 💡 ВИПРАВЛЕНО: Додано повну логіку оновлення зв'язків ProjectTechnology
        public async Task UpdateProjectAsync(Guid projectId, Guid ownerId, UpdateProjectDto updateDto)
        {
            // GetByIdAsync тепер завантажує ProjectTechnologies
            var project = await _projectRepository.GetByIdAsync(projectId); 
            ...
            // 2. Оновлення технологій (якщо вони були передані)
            if (updateDto.ProjectTechnologies != null)
            {
                ...
                // A. Видалити старі зв'язки (використовуючи завантажені ProjectTechnologies)
