...
namespace InternHub.API.Controllers
{
    ...
    public class AdminController : ControllerBase
    {
        ...
        [HttpGet("users")]
        public async Task<IActionResult> GetAllUsers(int pageNum = 1, int pageSize = 10)
        {
            var users = await _adminService.GetAllUsersAsync(pageNum, pageSize);
            return Ok(users);
        }
        ...
        {
            var companies = await _adminService.GetAllCompaniesAsync(pageNum, pageSize);
            return Ok(companies);
        }
        ...
        {
            var jobs = await _adminService.GetAllJobsAsync(pageNum, pageSize);
            return Ok(jobs);
        }
